"""
eval_four_policies.py  [v2b: path-B bypass]
-------------------------------------------
Changes vs v1 (original):
  [v2b] bypass_mask: True only when BOTH conditions hold:
        (1) sample has no retrieval context (use_context=False)
        (2) sample is NOT a hard subset (conflict / multi_entity)
        conflict and multi_entity samples always go through fusion_layer,
        preserving the representation quality they depend on.
        none and ambiguous samples with no context skip fusion,
        eliminating zero-vector noise.
  [v2b] None subset F1 fixed in reporting (compute_subset_metrics
        does not handle empty-tag samples; now computed manually).
  [v2b] Policy comparison table now shows None column correctly.

Usage:
  # Retrain with path-B bypass
  python eval_four_policies.py --stage train --use_bypass ^
      --output_dir F:\\stage4\\checkpoints\\retrieval_model_bypass

  # Eval bypass model
  python eval_four_policies.py --stage eval --use_bypass ^
      --output_dir F:\\stage4\\checkpoints\\retrieval_model_bypass ^
      --router_cache F:\\stage4\\router_cache\\test_router.jsonl

  # Eval original model (no bypass, backward compatible)
  python eval_four_policies.py --stage eval ^
      --output_dir F:\\stage4\\checkpoints\\retrieval_model ^
      --router_cache F:\\stage4\\router_cache\\test_router.jsonl
"""
...
class RetrievalAugmentedModel(nn.Module):
    ...
    bypass_mask=None (original / --use_bypass not set):
      All samples: [cls; zeros_or_ctx] -> fusion_layer -> classifier.

    bypass_mask=(B,) bool (--use_bypass set, path-B logic):
      True  -> sample has no context AND is not hard subset
               -> cls -> classifier directly (no zero-vector noise)
      False -> sample has context OR is conflict/multi_entity
               -> [cls; ctx_or_zeros] -> fusion_layer -> classifier
    """
    ...
    def forward(
        ...
        if bypass_mask is not None and bypass_mask.any():
            ...
            # Direct path: raw CLS -> classifier (no fusion noise)
            if len(bypass_idx) > 0:
                sent_logits[bypass_idx] = self.classifier(cls[bypass_idx])

            # Fusion path: [CLS; ctx] -> fusion_layer -> classifier
            if len(fusion_idx) > 0:
            ...
            # Original single path (bypass_mask=None or all False)
...
def load_router_cache(path: str) -> Dict[str, dict]:
    cache = {}
    ...
    print("[OK] Router cache: {} entries".format(len(cache)))
    return cache
...
def compute_all_subset_metrics(records: List[PredictionRecord]) -> dict:
    """
    Extends compute_subset_metrics with explicit handling for the none subset
    (samples with empty subset_tags), which the original util function misses.
    """
    # Named subsets from utils
    subset_metrics = compute_subset_metrics(records)

    # None subset: samples with no hard subset tags
...
# Hard subsets that always go through fusion (never bypass)
_HARD_SUBSETS = frozenset(["conflict", "multi_entity"])


class RetrievalDataset(Dataset):
    """
    Path-B bypass logic in __getitem__:
      bypass = (not use_context) AND (not is_hard_subset)
      where is_hard_subset = any tag in {conflict, multi_entity}

    This ensures:
      - conflict/multi_entity: always fusion (bypass=False)
      - none/ambiguous with no context: bypass fusion (bypass=True)
      - any sample with real context: always fusion (bypass=False)
    """
    ...
    def __init__(
        ...
        router_cache:           Optional[Dict[str, dict]] = None,
    ):
        ...
        self.router_cache           = router_cache or {}
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            elif self.policy == "router":
                signal = self.router_cache.get(s.sample_id, {
                    "should_retrieve": False,
                    "expert_type":     "base",
                    "confidence":      1.0,
                })
                if signal["expert_type"] == "context":
                    use_context = len(contexts) > 0
                else:
                    use_context = (
                        not signal["should_retrieve"] is False
        ...
        # [v2b] Path-B bypass logic:
        # bypass=True only when: no context AND not a hard subset
...
def evaluate(model, samples, tokenizer, cache, context_encoder, device,
             ...
             threshold=0.5, router_cache=None,
             use_bypass=False) -> List[PredictionRecord]:
    dataset = RetrievalDataset(
        samples, tokenizer, cache, context_encoder, device,
        policy=policy, threshold=threshold, max_length=max_length,
        is_train=False, router_cache=router_cache,
    )
...
def main(args):
    ...
    if not best_model_path.exists():
        print("[FAIL] best_model.pt not found. Run --stage train first.")
        return
    model.load_state_dict(torch.load(best_model_path, map_location=device))
    model.to(device)

    router_cache = None
    if args.router_cache and Path(args.router_cache).exists():
        router_cache = load_router_cache(args.router_cache)

    policies = ["none", "always_on", "conditional"]
    if router_cache is not None:
        policies.append("router")
    else:
        print("[INFO] No router_cache -- evaluating 3 policies.")
    ...
    for policy in policies:
        records_main = evaluate(
            ...
            router_cache=router_cache, use_bypass=args.use_bypass)
        ...
        # [v2b] Use extended subset metrics that include none subset
        subset_metrics = compute_all_subset_metrics(records_main)
    ...
    # [v2b] Fixed policy comparison table with correct None column
    ...
    print("\n[OK] Saved: {}".format(result_file))
    print("[INFO] Compare: retrieval_policy_comparison_nbypass.json vs _bypass.json")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--stage",      choices=["cache", "train", "eval"], default="train")
    ...
    parser.add_argument("--router_cache", default=None)
